...
class DocumentForensicsWebApp:
    ...
    def get_analysis_results(self, document_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/analysis/{document_id}/results",
    ...
    def render_analysis_results(self, results: Dict[str, Any]):
        ...
        with col1:
            # Display risk with colored emoji
            risk_emoji = {
                "low": "🟢",
                "medium": "🟡", 
                "high": "🟠",
            ...
            st.metric("Overall Risk", f"{risk_emoji} {overall_risk.upper()}")
        ...
        if uploaded_files:
            ...
            if st.button("Start Batch Analysis", type="primary"):
                with st.spinner("Uploading and analyzing documents..."):
                    ...
                    if document_ids:
                        st.success(f"✅ Batch processing started for {len(document_ids)} documents!")
                        st.session_state.batch_document_ids = document_ids
                        st.rerun()
                    else:
                        st.error("Failed to upload documents")
        
        # Batch status
        if hasattr(st.session_state, 'batch_document_ids') and st.session_state.batch_document_ids:
            st.subheader("Batch Status")
            
            document_ids = st.session_state.batch_document_ids
            completed = 0
            failed = 0
            processing = 0
            ...
            with status_container:
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total", len(document_ids))
                with col2:
                ...
                st.subheader("Individual Document Status")
                for doc_id in document_ids:
                    status_data = self.get_document_status(doc_id)
                    if status_data:
                        status = status_data.get("status", "unknown")
                        ...
                        with st.expander(f"Document {doc_id} - {status.upper()}"):
                ...
                if processing > 0:
                    st.info("⏳ Analysis in progress... Page will auto-refresh")
                    time.sleep(2)
                    st.rerun()
                
                # Clear batch button
                if st.button("Clear Batch"):
                    del st.session_state.batch_document_ids
                    st.rerun()
